...
- ~~La relación bidireccional real (RF-09/RF-14) no se implementa en esta iteración: requiere decidir cómo se vincula un `Articulo` con una `Obligation`.~~ **Decidido e implementado el 26-ago-2026 (#110).** La columna ya existía —`obligations.article_compliance_id`, con su clave foránea desde `01_schema.sql`— y lo que faltaba era algo que la escribiera y la leyera. Los dos sentidos: `POST /compliance/article-compliance/{id}/obligations` crea desde la matriz, `PUT|DELETE /obligations/{id}/matrix-link` ata y suelta una obligación que ya existía. El chip "Próximamente" del detalle se reemplazó por el enlace real hacia el artículo de origen; cuando la obligación nació libremente no se muestra nada, porque eso es un estado legítimo y no una carencia.
- **Y al medirlo apareció un agujero que no estaba en el ticket:** `POST /obligations/` aceptaba `article_compliance_id` sin comprobarlo. Como las claves foráneas no pasan por RLS, la empresa B podía colgar su obligación de la evaluación de la empresa A — medido: **422 con un id inventado y 201 con uno real ajeno**, y la fila quedaba escrita. Ahora las dos respuestas son idénticas.
