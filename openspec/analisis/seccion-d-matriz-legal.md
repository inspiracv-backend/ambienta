...
- ~~La relación bidireccional Matriz Legal ↔ Obligaciones (RF-09, RF-14) no se implementa en esta iteración.~~ **Cerrado el 26-ago-2026 (#110).** El vínculo se cuelga de la **evaluación** del artículo (`article_compliance`) y no del artículo del catálogo, que es global y no diría de qué empresa ni de qué planta es. El botón "Generar obligación desde este artículo" está en el diálogo de evaluar y llega a la base; la norma, la planta y el responsable se derivan de la evaluación en vez de pedirse. Ver `apps/api/app/services/vinculo_matriz_obligacion.py`.
